...
export const TrendChart: React.FC<TrendChartProps> = ({ data }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < data.length; i++) {
      const x = padding + (i * xStep);
      const y = padding + chartHeight - ((data[i].supportPercentage / 100) * chartHeight);
  ...
  const handleMouseMove = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const pointY = padding + chartHeight - ((data[i].supportPercentage / 100) * chartHeight);
  ...
  if (data.length === 0) {
    return (
      <div className="flex items-center justify-center h-96 text-gray-500">
        No historical data available. Run <code>node --run generate:historical</code> to collect data.
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      <div className="mb-4">
        <h2 className="text-xl font-semibold mb-2">Workerd Node.js API Support Over Time</h2>
        <p className="text-sm text-gray-600">
          Percentage of Node.js APIs supported by Cloudflare Workers runtime (workerd) by month
        </p>
      </div>
      ...
      <div className="mt-4 text-sm text-gray-600">
        <p>
          <strong>Latest:</strong> {data[data.length - 1]?.supportPercentage}% support 
          ({data[data.length - 1]?.supportedApis}/{data[data.length - 1]?.totalApis} APIs)
        </p>
        {data.length > 1 && (
          <p>
            <strong>Improvement:</strong> +{(data[data.length - 1]?.supportPercentage - data[0]?.supportPercentage).toFixed(1)}% 
            since {data[0]?.date}
          </p>
        )}
      </div>
