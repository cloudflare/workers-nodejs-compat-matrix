...
async function fetchAvailableWorkerdVersions() {
  ...
  try {
    const result = shell.exec(`npm view workerd versions --json`, { silent: true });
    if (result.code !== 0) {
    ...
    console.error("Failed to fetch npm versions, falling back to GitHub releases");
...
async function fetchWorkerdReleasesFromGitHub() {
  ...
  while (true) {
    ...
    if (pageReleases.some(release => new Date(release.published_at).getFullYear() < 2024)) {
      break;
    }
  }
...
function selectMonthlySnapshots(versions) {
  ...
  for (const version of versions) {
    ...
    if (!monthlyReleases.has(monthKey) || 
  ...
  return Array.from(monthlyReleases.values())
    .sort((a, b) => new Date(a.versionDate) - new Date(b.versionDate)); // oldest first
}
...
async function findMaxSupportedCompatibilityDate(workerdVersion) {
  ...
  try {
    ...
    const testResult = shell.exec(`node workerd/dump.mjs ${versionDate}`, { silent: true });
    console.log(`Test result code: ${testResult.code}`);
    if (testResult.stderr) console.log(`Test stderr: ${testResult.stderr.substring(0, 200)}...`);
  ...
  for (let daysBack = 0; daysBack <= 30; daysBack++) {
    ...
    try {
      ...
      const testResult = shell.exec(`node workerd/dump.mjs ${compatDate}`, { silent: true });
      if (testResult.code === 0) {
...
async function calculateSupportPercentage() {
  ...
  const visit = (node, path) => {
    ...
    for (const [key, childNode] of Object.entries(node)) {
      ...
      if (isObject(childNode)) {
        ...
        if (targetValue === "stub" || isPartOfMockModule(workerdData, keyPath)) {
          supportStatus = "stub";
  ...
  // Count supported APIs (supported + stub + mismatch all count as "present")
  let supportedApis = 0;
  for (const status of rows) {
    if (status === "supported" || status === "stub" || status === "mismatch") {
      supportedApis += 1;
    }
  }
...
async function collectHistoricalData(monthlySnapshots) {
  ...
  console.log(`Collecting data for ${monthlySnapshots.length} monthly snapshots...`);
  ...
  for (const [index, snapshot] of monthlySnapshots.entries()) {
    ...
    try {
      ...
      const installResult = shell.exec("pnpm install --dir ./workerd/", { silent: false });
      ...
      const workerdPathResult = shell.exec("pnpm --dir ./workerd/ bin workerd", { silent: true });
      console.log(`Workerd binary path result: ${workerdPathResult.stdout.trim()}`);
      ...
      if (!shell.test('-f', workerdBinaryPath)) {
        console.log(`Binary not found at ${workerdBinaryPath}, trying direct approach...`);
        // Try to find workerd binary directly
        const directPath = path.join(__dirname, "workerd", "node_modules", ".bin", "workerd");
      ...
      const compatDate = await findMaxSupportedCompatibilityDate(snapshot.version);
      ...
      const result = shell.exec(`node workerd/dump.mjs ${compatDate}`, { silent: false });
      ...
      if (result.code !== 0) {
        console.log(`⚠️  Failed to scan APIs for ${snapshot.version} - SKIPPING`);
      ...
      try {
        ...
        console.error(`Failed to calculate support percentage: ${error.message}`);
        continue;
      }
      ...
      console.log(`${snapshot.date}: ${supportPercentage}% (${supportedApis}/${totalApis})`);
...
async function main() {
  try {
    console.log("Starting historical data collection for workerd Node.js API support...\n");
    
    // Fetch available versions from npm
    const availableVersions = await fetchAvailableWorkerdVersions();
    console.log(`Found ${availableVersions.length} versions available in npm since 2024`);
    ...
    console.log(`📈 Support improved from ${historicalData[0]?.supportPercentage}% to ${historicalData[historicalData.length - 1]?.supportPercentage}%`);
